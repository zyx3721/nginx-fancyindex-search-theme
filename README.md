...
搜索仅覆盖文件名和相对路径，不读取文件正文。SQLite FTS5 使用不区分大小写的词前缀匹配，多个词需要同时命中。例如查询 `annual rep` 可匹配 `Annual Report 2026.pdf`。

范围由 `path` 控制：前端会把浏览器当前 `window.location.pathname` 作为 `path` 传给 API；`/archive/` 只会返回 `/archive/` 以及其后代，不会返回同级或父级目录结果。单次最多返回 100 条，以保护接口和浏览器渲染性能。
...
| `files_after_*` 触发器 | `files` 的插入、删除，以及名称/路径更新事件。 | 让 `files_fts` 与主表保持同步；文件大小、修改时间或扫描标记更新不会重建全文索引。 |

首次执行会创建上述结构并写入所有可遍历条目。后续执行**不会删除数据库或重新创建表**，但会采用“全量遍历 + 增量同步”流程：
...
首次建立索引会完整遍历文件树，耗时受文件数、目录深度、文件系统类型和存储 IOPS 影响。搜索请求只读 SQLite 索引，其延迟不随目录深度线性增长。
...
| `limit` | 否 | 返回数量，范围 1 到 100；默认 100。 |
...
```bash
python3 -m unittest discover -s search/tests -v
